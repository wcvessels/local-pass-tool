...
const expectedCommands = [...new Set(
  Object.values(expectedCapabilities).flat().map((permission) => permission.slice("allow-".length).replaceAll("-", "_")),
)].sort();
const buildSource = await read("src-tauri/build.rs");
const mainSource = await read("src-tauri/src/main.rs");
const buildCommands = [...(buildSource.match(/const COMMANDS:[\s\S]*?=\s*&\[([\s\S]*?)\];/)?.[1] ?? "").matchAll(/"([a-z_]+)"/g)].map((match) => match[1]).sort();
const runtimeCommands = (mainSource.match(/\.invoke_handler\(tauri::generate_handler!\[([\s\S]*?)\]\)/)?.[1] ?? "").split(",").map((command) => command.trim()).filter(Boolean).sort();
check(same(buildCommands, expectedCommands), "build command list must match exact LocalPass capabilities");
check(same(runtimeCommands, expectedCommands), "runtime command list must match exact LocalPass capabilities");
const opacityPermission = await read("src-tauri/permissions/autogenerated/set_collapsed_opacity.toml");
check(
  same([...opacityPermission.matchAll(/^commands\.(allow|deny)\s*=\s*(.+)$/gm)].map((match) => [match[1], canonicalToml(match[2])]), [
    ["allow", '["set_collapsed_opacity"]'],
    ["deny", '["set_collapsed_opacity"]'],
  ]),
  "collapsed opacity permission must allow or deny only its single command",
);
